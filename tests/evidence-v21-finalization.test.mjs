  ...
  return {
    async signDigest(input) {
      ...
      return {
        ...
        signedAt: '2026-09-11T10:05:01.000Z',
      };
    },
  };
}
...
const fixture = {
  ...
  closedAt: '2026-09-11T10:04:59.000Z',
  environment: 'PREVIEW',
  platform: 'Docubox',
  ...
  document: {
    ...
    finalHash: pdfHash,
    preparedAt: '2026-09-11T10:01:00.000Z',
    flowStartedAt: '2026-09-11T10:02:00.000Z',
    metadata,
  ...
  participants: [
    {
      ...
      firmanteId: '66666666-6666-4666-8666-666666666666',
      ...
      order: 1,
      required: true,
      expectedMethod: 'autografa_digital',
      participationStatus: 'firmado',
      invitationAt: '2026-09-11T10:02:00.000Z',
      firstAccessAt: '2026-09-11T10:03:00.000Z',
  ...
  signatures: [
    {
      ...
      signedObjectHash: pdfHash,
      capturedAt: '2026-09-11T10:03:59.000Z',
      signedAt: '2026-09-11T10:04:00.000Z',
      ...
      context: {
        ...
        userAgent: 'Docubox Test Agent',
        deviceInfo: 'desktop; 1920x1080',
      },
      consent: {
        textVersion: consent.SIGNATURE_CONSENT_VERSION,
        textHash: consent.SIGNATURE_CONSENT_SHA256,
        accepted: true,
        acceptedAt: '2026-09-11T10:03:58.000Z',
      },
      autograph: {
        captureId: '88888888-8888-4888-8888-888888888888',
        strokesHash: hash('strokes'),
        imageHash: hash('image'),
        combinedHash: hash(`${hash('image')}${hash('strokes')}`),
  ...
  events: [
    {
      ...
      payloadHash: hash('{}'),
      chainMaterial: eventOneMaterial,
      previousSourceHash: '0'.repeat(64),
      sourceEventHash: eventOneHash,
    },
    {
      ...
      result: 'SUCCESS',
      occurredAt: '2026-09-11T10:04:00.000Z',
      actorRef: 'user:signer',
      objectRef: 'document:44444444-4444-4444-8444-444444444444',
      eventCategory: 'SIGNATURE',
      actorType: 'PARTICIPANT',
      documentHash: pdfHash,
      payloadHash: hash('{"signed":true}'),
      chainMaterial: eventTwoMaterial,
      previousSourceHash: eventOneHash,
      sourceEventHash: eventTwoHash,
    },
  ],
  timestamps: [
    {
      ...
      objectHash: pdfHash,
      provider: 'TSA TEST',
      issuedAt: '2026-09-11T10:04:30.000Z',
      messageImprint: pdfHash,
      artifactRef: 'storage:certification-artifacts/timestamp.tst',
      artifactHash: hash('rfc3161-token'),
    ...
    },
    {
      ...
      issuedAt: '2026-09-11T10:04:40.000Z',
      artifactRef: 'storage:blockchain-evidence/proof.ots',
      artifactHash: hash('ots-proof'),
      manifestHash: hash('ots-manifest'),
      validationStatus: 'pending',
    },
  ],
  nom151: {
    status: 'pending',
    requestId: 'nom-request-1',
    requestedAt: '2026-09-11T10:04:45.000Z',
    hashSubmitted: pdfHash,
  },
};
...
test('V2.1 generates XSD-valid XML with no XMLDSig claim and round-trips', async () => {
  ...
  assert.equal(
    rebuiltRoot.metadataSnapshotHash,
    built.package.evidenceRoot.metadataSnapshotHash,
    JSON.stringify({ rebuiltRoot, stored: built.package.evidenceRoot })
  );
  assert.equal(
    rebuiltRoot.evidenceEventRootHash,
    built.package.evidenceRoot.evidenceEventRootHash,
    JSON.stringify({ rebuiltRoot, stored: built.package.evidenceRoot })
  );
  assert.equal(
    rebuiltRoot.signaturesDigest,
    built.package.evidenceRoot.signaturesDigest,
    JSON.stringify({ rebuiltRoot, stored: built.package.evidenceRoot })
  );
  assert.equal(
    rebuiltRoot.packageCoreDigest,
    built.package.evidenceRoot.packageCoreDigest,
    JSON.stringify({ rebuiltRoot, stored: built.package.evidenceRoot })
  );
...
test('V2.1 represents a completed in-person act without exposing kiosk secrets', async () => {
  const inPerson = structuredClone(fixture);
  inPerson.signatures[0].participation = {
    mode: 'in_person',
    participantReference: 'participant-reference:12121212-1212-4212-8212-121212121212',
    authentication: {
      method: 'authenticated_session',
      result: 'verified',
      verifiedAt: '2026-09-11T10:03:30.000Z',
      evidenceRef: 'completion-attempt:13131313-1313-4313-8313-131313131313',
    },
    signingMethod: 'autografa_digital',
    consentRef: `consent-sha256:${consent.SIGNATURE_CONSENT_SHA256}`,
    completion: {
      attemptRef: 'completion-attempt:13131313-1313-4313-8313-131313131313',
      correlationId: '14141414-1414-4414-8414-141414141414',
      committedAt: '2026-09-11T10:04:00.000Z',
      responseRef: 'participation-response:15151515-1515-4515-8515-151515151515',
      canonicalEventRef: 'operational-event:16161616-1616-4616-8616-161616161616',
    },
    inPersonSession: {
      sessionRef: 'in-person-session:17171717-1717-4717-8717-171717171717',
      status: 'completed',
      createdAt: '2026-09-11T10:02:30.000Z',
      startedAt: '2026-09-11T10:03:00.000Z',
      completedAt: '2026-09-11T10:04:00.000Z',
    },
  };

  const first = await generator.createEvidenceV2Package(inPerson, testSigner());
  const second = await generator.createEvidenceV2Package(inPerson, testSigner());
  const parsed = parser.parseEvidenceV2Xml(first.xml);
  assert.deepEqual(parsed.signatures[0].participation, inPerson.signatures[0].participation);
  assert.equal(first.package.evidenceRoot.value, second.package.evidenceRoot.value);
  assert.equal(readiness.validateEvidenceV21Readiness(first.package).status, 'READY');
  assert.match(first.xml, /<Participation mode="in_person">/);
  assert.match(first.xml, /<InPersonSession[^>]+status="completed">/);
  assert.doesNotMatch(first.xml, /kiosk[_-]?token|otp[_-]?code|cookie|bearer|password/i);

  const tampered = first.xml.replace('mode="in_person"', 'mode="remote"');
  assert.equal(verifier.verifyEvidenceV2Xml(tampered).overall, 'invalid');

  const cancelled = structuredClone(first.package);
  cancelled.signatures[0].participation.inPersonSession.status = 'cancelled';
  assert.match(
    readiness.validateEvidenceV21Readiness(cancelled).codes.join(','),
    /IN_PERSON_PARTICIPATION_INCOMPLETE/
  );
});

test('remote and historical V2.1 packages do not require in-person fields', async () => {
  const historical = await generator.createEvidenceV2Package(fixture, testSigner());
  assert.doesNotMatch(historical.xml, /<Participation/);
  assert.equal(readiness.validateEvidenceV21Readiness(historical.package).status, 'READY');

  const remote = structuredClone(fixture);
  remote.signatures[0].participation = {
    mode: 'remote',
    participantReference: 'participant-reference:12121212-1212-4212-8212-121212121212',
    authentication: {
      method: 'authenticated_session',
  ...
  const built = await generator.createEvidenceV2Package(remote, testSigner());
  assert.match(built.xml, /<Participation mode="remote">/);
  assert.doesNotMatch(built.xml, /<InPersonSession/);
  assert.equal(readiness.validateEvidenceV21Readiness(built.package).status, 'READY');
});

test('V2.1 round-trips additive signing group and delegation governance', async () => {
  const governed = structuredClone(fixture);
  governed.signatures[0].participation = {
    mode: 'remote',
    participantReference: 'participant-reference:12121212-1212-4212-8212-121212121212',
    authentication: {
      method: 'authenticated_session',
    ...
    governance: {
      signingGroupRef: 'signing-group:18181818-1818-4818-8818-181818181818',
      groupMemberSlotRef: 'group-member-slot:20202020-2020-4020-8020-202020202020',
      completionPolicy: 'ANY_ONE',
      delegationRef: 'delegation:19191919-1919-4919-8919-191919191919',
      effectiveActorRef: 'user:delegate',
      actionRole: 'signature',
      eligibleParticipantRefs: [
        'participant-reference:12121212-1212-4212-8212-121212121212',
        'participant-reference:22222222-2222-4222-8222-222222222222',
      ],
      winnerParticipantRef: 'participant-reference:12121212-1212-4212-8212-121212121212',
      originalParticipantRef: 'participant-reference:12121212-1212-4212-8212-121212121212',
      delegateUserRef: 'user:delegate',
      delegateParticipantRef: 'participant-reference:22222222-2222-4222-8222-222222222222',
      delegationCreatedByRef: 'user:admin',
      delegationPolicy: 'AUTHORIZED_MEMBERS',
      delegationReason: 'Cobertura autorizada durante ausencia.',
      delegationCreatedAt: '2026-09-11T09:00:00.000Z',
      delegationCompletedAt: '2026-09-11T10:04:00.000Z',
    },
  };
  const built = await generator.createEvidenceV2Package(governed, testSigner());
  const parsed = parser.parseEvidenceV2Xml(built.xml);
  assert.deepEqual(
    parsed.signatures[0].participation.governance,
    governed.signatures[0].participation.governance
  );
  assert.match(built.xml, /<EligibleParticipantRefs>/);
  assert.match(built.xml, /<WinnerParticipantRef>/);
  assert.match(built.xml, /<GroupMemberSlotRef>/);
  assert.match(built.xml, /<DelegateParticipantRef>/);
  assert.match(built.xml, /<DelegationPolicy>AUTHORIZED_MEMBERS<\/DelegationPolicy>/);
  assert.equal(
    verifier.verifyEvidenceV2Xml(built.xml).overall,
...
test('EvidenceRoot has a stable golden vector and covers every required digest', async () => {
  ...
  assert.equal(first.package.evidenceRoot.value, second.package.evidenceRoot.value);
  assert.equal(
    first.package.evidenceRoot.value,
    'f5e2c145dac525e69b9ad8a5da2c00443d65b3e68ea26250867ed8c80c81d35e'
  );
...
test('readiness enforces stable autograph identity, consent and technical evidence', async () => {
  ...
  assert.deepEqual(readiness.validateEvidenceV21Readiness(built.package), {
    status: 'READY',
    codes: [],
    pendingSupplements: ['OPENTIMESTAMPS', 'NOM151'],
  });
...
test('tampering PDF, XML metadata, event chain or signature is rejected', async () => {
  const built = await generator.createEvidenceV2Package(fixture, testSigner());
  const badPdf = await verifier.verifyEvidenceV2Package({
    xml: built.xml,
...
test('manifest is deterministic and reports pending and non-applicable items', () => {
  const input = {
    ...
    packageRow: {
      ...
      evidence_root_sha256: 'a'.repeat(64),
...
test('finalization contracts cover idempotency, concurrency, retry, storage and supplements', () => {
  const migration = readFileSync(
    'supabase/migrations/20260911013223_evidence_v21_finalization.sql',
    'utf8'
  );
