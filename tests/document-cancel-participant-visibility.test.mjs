...
test('participant portal resolves a real document name without caching the capability response', async () => {
  ...
  assert.match(portalPage, /Tienes una invitación para participar en este documento/);
  ...
  assert.match(portalPage, /Acceder con mi cuenta/);
  assert.match(portalPage, /Restablecer contraseña/);
  assert.match(portalPage, /Cuento con mi usuario y contraseña para revisar y participar\./);
  assert.match(portalPage, /He firmado previamente pero no recuerdo mi contraseña\./);
  assert.match(portalPage, /Necesito ayuda con el proceso de participación en el documento\./);
  assert.match(portalPage, /Crear mi acceso/);
  assert.match(portalPage, /flex flex-1 items-center justify-center py-10/);
  assert.match(portalPage, /lg:grid-cols-\[minmax\(320px,0\.7fr\)_minmax\(0,1\.3fr\)\]/);
  assert.match(portalPage, /<section className="mb-10">/);
