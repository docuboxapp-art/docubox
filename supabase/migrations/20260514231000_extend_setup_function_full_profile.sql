-- Migration: Extend setup_free_workspace_and_subscription to save all profile fields
-- Adds: nombre, apellido_paterno, apellido_materno, telefono, fecha_nacimiento, sexo,
--       tipo_identificacion, efirma_rfc, efirma_serial, efirma_nombre, efirma_vigencia_fin
...
CREATE OR REPLACE FUNCTION public.setup_free_workspace_and_subscription(
    ...
    p_tipo_identificacion TEXT DEFAULT NULL,
    -- e.Firma specific fields
    p_efirma_rfc TEXT DEFAULT NULL,
    p_efirma_serial TEXT DEFAULT NULL,
    p_efirma_nombre TEXT DEFAULT NULL,
    p_efirma_vigencia_fin TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Update user profile with ALL available data from registration
    ...
    SET
        ...
        -- Extended personal fields from CURP / biometric enrollment
        ...
        telefono             = COALESCE(p_telefono, p_phone, telefono),
        -- e.Firma specific fields (only set when identity method is efirma)
        efirma_rfc           = COALESCE(p_efirma_rfc, efirma_rfc),
        efirma_serial        = COALESCE(p_efirma_serial, efirma_serial),
        efirma_nombre        = COALESCE(p_efirma_nombre, efirma_nombre),
        efirma_vigencia_fin  = COALESCE(p_efirma_vigencia_fin, efirma_vigencia_fin),
        efirma_linked_at     = CASE
                                 WHEN p_efirma_serial IS NOT NULL AND efirma_linked_at IS NULL
                                 THEN CURRENT_TIMESTAMP
                                 ELSE efirma_linked_at
                               END,
