-- Keep ordinary session validation read-only. A row lock is required only when
-- explicit user activity advances the inactivity deadline or when a request is
-- about to record an inactivity timeout and must recheck against concurrent input.
...
BEGIN
  ...
  IF CURRENT_TIMESTAMP >= v_inactivity_expired_at THEN
    -- A user action may have advanced the deadline after the read-only check.
    -- Lock and re-read before recording an inactivity timeout.
