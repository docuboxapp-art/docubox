-- Restore the Fase 5 concurrency guarantee after the 20-minute timeout update.
-- Ordinary navigation only reads the inactivity deadline. It must not serialize
-- concurrent tabs behind a row lock. A lock is still required when activity is
-- recorded or when an expiry decision needs to win a race with that activity.
...
BEGIN
  ...
  v_inactivity_timeout_seconds := CASE WHEN v_is_privileged THEN 600 ELSE 1200 END;
      ...
      IF FOUND AND EXISTS (SELECT 1 FROM public.user_profiles profiles WHERE profiles.id = v_user_id) THEN
        INSERT INTO public.auth_security_events (
          ...
          CASE WHEN v_is_privileged
            ...
            ELSE 'Sesión cerrada automáticamente por inactividad (20 minutos)'
          END,
          jsonb_build_object('source', 'server_session_policy', 'session_id', v_session_id)
        );
      END IF;
