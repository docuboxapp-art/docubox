-- Only super administrators expire after 15 minutes of human inactivity.
-- Other accounts retain 30 minutes and the read-only session check remains unchanged.
-- The absolute 4-hour / 8-hour limits remain unchanged.
...
BEGIN
  ...
  SELECT COALESCE(users.is_super_admin, FALSE)
      OR COALESCE(roles.role_key = 'DOCUBOX_SUPER_ADMIN', FALSE),
    COALESCE(users.is_super_admin, FALSE) OR staff.user_id IS NOT NULL
  INTO v_is_super_admin, v_is_privileged
  FROM auth.users users
  LEFT JOIN public.platform_staff staff
    ON staff.user_id = users.id
    AND staff.status = 'active'
    AND (staff.valid_until IS NULL OR staff.valid_until > CURRENT_TIMESTAMP)
  LEFT JOIN public.platform_roles roles ON roles.id = staff.role_id
  ...
  IF v_last_user_activity_at IS NULL THEN
    RETURN QUERY SELECT FALSE, 'SESSION_NOT_FOUND', v_is_privileged,
      v_inactivity_timeout_seconds, v_session_started_at, NULL::TIMESTAMPTZ,
      NULL::TIMESTAMPTZ, v_absolute_expired_at;
    RETURN;
  END IF;
  ...
  IF CURRENT_TIMESTAMP >= v_inactivity_expired_at THEN
    -- Re-check under lock only when the read-only fast path observes an expiry.
    IF NOT p_record_user_activity THEN
      SELECT activity.last_user_activity_at
      INTO v_last_user_activity_at
      FROM public.docubox_session_activity activity
      WHERE activity.session_id = v_session_id
    ...
    IF CURRENT_TIMESTAMP >= v_inactivity_expired_at THEN
      ...
      IF FOUND AND EXISTS (SELECT 1 FROM public.user_profiles profiles WHERE profiles.id = v_user_id) THEN
        INSERT INTO public.auth_security_events (
          ...
          CASE
            WHEN v_is_super_admin THEN 'Sesión de superadministrador cerrada automáticamente por inactividad (15 minutos)'
            WHEN v_is_privileged THEN 'Sesión administrativa cerrada automáticamente por inactividad (30 minutos)'
            ELSE 'Sesión cerrada automáticamente por inactividad (30 minutos)'
          END,
          jsonb_build_object('source', 'server_session_policy', 'session_id', v_session_id)
        );
      END IF;
