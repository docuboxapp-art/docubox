export type LuciaIntent =
  | 'user_profile' |'user_profile_sensitive' |'user_usage' |'billing_status' |'user_created_documents' |'user_assigned_documents' |'user_participations' |'document_types_assigned' |'document_status_search' |'signature_status' |'pending_tasks' |'notifications_search' |'activity_history' |'expediente_search' |'contacts_search' |'templates_help' |'forms_help' |'document_content_search' |'document_summary' |'legal_analysis' |'compliance_analysis' |'contract_generation' |'signing_help' |'external_participant_help' |'configuration_security' |'reports_analysis' |'general_help';
...
export type LuciaScope =
  | 'auth' |'registration' |'email_verification' |'password_recovery'
  | 'dashboard' |'documents' |'create_document' |'document_viewer' |'signing' |'participations' |'participation_requests' |'contacts' |'external_participant' |'external_registration' |'public_form' |'forms' |'form_builder' |'templates' |'reports' |'billing' |'profile' |'settings' |'mobile_enrollment' |'mobile_upload' |'mobile_id_capture' |'notifications' |'pending_tasks' |'integrations' |'signing_help_page' |'workspace';
...
export function getScopeFromRoute(route: string): LuciaScope {
  ...
  if (r === '/sign-up-login-screen') return 'auth';
  ...
  if (r === '/documents-dashboard') return 'dashboard';
  ...
  if (r === '/participation-requests') return 'participation_requests';
  ...
  if (r === '/pending-tasks') return 'pending_tasks';
...
export function buildRouteContext(
  ...
  const routeMap: Record<LuciaScope, Omit<RouteContext, 'route' | 'current_resource_ids' | 'ui_state'>> = {
    auth: {
    ...
    dashboard: {
      ...
      available_fields: ['total_documents', 'pending_documents', 'signed_documents', 'rejected_documents', 'activity_feed', 'expiring_soon'],
      available_actions: ['abrir documento', 'revisar actividad', 'generar reporte', 'ver documentos pendientes', 'ver documentos vencidos'],
    },
    documents: {
      ...
      available_fields: ['nombre', 'estado', 'fecha', 'participantes', 'etiquetas', 'carpeta', 'tipo_documento'],
      available_actions: ['ver', 'descargar', 'cancelar', 'reenviar recordatorio', 'filtrar por estado', 'filtrar por carpeta', 'personalizar vista'],
    },
    create_document: {
      ...
      available_fields: ['nombre_documento', 'tipo_documento', 'participantes', 'rol_participante', 'orden_firma', 'fecha_limite', 'recordatorios', 'carpeta', 'etiquetas'],
      available_actions: ['subir archivo', 'agregar participante', 'configurar flujo', 'establecer fecha límite', 'enviar documento', 'guardar borrador'],
    },
    document_viewer: {
      ...
      available_fields: ['estado', 'participantes', 'fechas', 'notas', 'historial', 'contenido_pdf'],
      available_actions: ['resumir documento', 'detectar riesgos', 'mostrar historial', 'revisar participantes', 'agregar nota', 'descargar documento', 'ver auditoría'],
    },
    signing: {
      ...
      available_fields: ['estado_firma', 'campos_firma', 'metodo_firma', 'similitud_biometrica', 'otp_sms'],
      available_actions: ['firmar con autógrafa', 'firmar con e.firma', 'validar OTP', 'capturar selfie', 'ver documento'],
    },
    participations: {
    ...
    contacts: {
      ...
      available_actions: ['buscar contacto', 'agregar contacto', 'editar contacto', 'eliminar contacto', 'invitar a documento'],
    },
    ...
    forms: {
      ...
      available_actions: ['crear formulario', 'editar formulario', 'previsualizar', 'publicar formulario'],
    },
    form_builder: {
      ...
      available_actions: ['agregar campo', 'reordenar campos', 'configurar propiedades', 'previsualizar', 'guardar formulario'],
    },
    templates: {
      ...
      available_actions: ['crear plantilla', 'editar plantilla', 'insertar variable', 'previsualizar', 'usar plantilla'],
    },
    reports: {
      ...
      available_actions: ['generar reporte', 'exportar CSV', 'exportar PDF', 'filtrar por fecha', 'filtrar por usuario'],
    },
    billing: {
      ...
      available_fields: ['plan_actual', 'documentos_usados', 'documentos_limite', 'historial_pagos', 'fecha_renovacion'],
      available_actions: ['ver plan actual', 'cambiar plan', 'ver historial de pagos', 'descargar factura'],
    },
    profile: {
      ...
      available_fields: ['nombre', 'email', 'telefono', 'RFC', 'CURP', 'firma_autografa', 'domicilio_fiscal', 'regimen_fiscal'],
      available_actions: ['actualizar perfil', 'cambiar contraseña', 'revisar eFirma', 'actualizar firma autógrafa', 'ver datos fiscales'],
    },
    settings: {
      ...
      available_fields: ['nombre_workspace', 'logo', 'zona_horaria', '2fa', 'sesiones_activas', 'notificaciones'],
      available_actions: ['actualizar workspace', 'activar 2FA', 'gestionar sesiones', 'configurar notificaciones', 'gestionar roles'],
    },
    mobile_enrollment: {
      ...
      available_actions: ['capturar selfie', 'capturar identificación', 'validar CURP', 'completar enrolamiento'],
    },
    mobile_upload: {
    ...
    notifications: {
      ...
      available_actions: ['marcar como leída', 'eliminar notificación', 'filtrar por tipo', 'marcar todas como leídas'],
    },
    pending_tasks: {
      ...
      available_actions: ['ir a firmar', 'ir a revisar', 'ir a aprobar', 'filtrar por prioridad', 'ver documento'],
    },
    integrations: {
      ...
      available_actions: ['instalar integración', 'desinstalar integración', 'ver detalles', 'configurar integración'],
    },
    signing_help_page: {
...
export const ROUTE_ACTION_INTENTS: Partial<Record<LuciaScope, Record<string, ActionIntent>>> = {
  document_viewer: {
    ...
    'detectar riesgos': {
      ...
      mode: 'rag',
      question: 'Detecta y lista los riesgos legales, cláusulas problemáticas u obligaciones importantes de este documento.',
    },
    'revisar participantes': {
      intent: 'user_participations',
      mode: 'structured',
      question: 'Muéstrame el estado actual de todos los participantes de este documento: quién ha firmado, quién está pendiente y quién ha rechazado.',
    },
    'mostrar historial': {
    ...
    'ver auditoría': {
      intent: 'activity_history',
      mode: 'structured',
      question: 'Muéstrame el registro de auditoría de este documento con todos los eventos registrados.',
    },
    'agregar nota': {
      intent: 'general_help',
      mode: 'structured',
...
export function matchRouteAction(question: string, scope: LuciaScope): ActionIntent | null {
  ...
  for (const [actionKey, actionIntent] of Object.entries(scopeActions)) {
    const normalizedKey = actionKey.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  ...
  if (scope === 'document_viewer') {
    ...
    if (q.includes('riesgo') || q.includes('problema') || q.includes('peligro') || q.includes('clausula') || q.includes('legal')) {
      return scopeActions['detectar riesgos'];
    }
    if (q.includes('participante') || q.includes('firmante') || q.includes('quien firmo') || q.includes('quien ha firmado') || q.includes('estado de firma') || q.includes('falta firmar')) {
      return scopeActions['revisar participantes'];
    }
    if (q.includes('historial') || q.includes('actividad') || q.includes('auditoria') || q.includes('log') || q.includes('eventos')) {
...
const SENSITIVE_PROFILE_KEYWORDS = [
  'mi curp', 'curp', 'mi rfc', 'rfc', 'mi teléfono', 'mi telefono',
  'mi número de teléfono', 'mi numero de telefono', 'mi domicilio',
  'mi dirección fiscal', 'mi direccion fiscal', 'mi domicilio fiscal',
  'datos fiscales', 'datos personales', 'mi código postal', 'mi codigo postal',
  'mi colonia', 'mi municipio', 'mi estado', 'mi calle',
  'datos de mi perfil', 'información personal', 'informacion personal',
];

const ACTIVITY_HISTORY_KEYWORDS = [
  'historial', 'actividad', 'actividades', 'cambios', 'movimientos',
  'bitácora', 'bitacora', 'auditoría', 'auditoria',
  'quién creó', 'quien creo', 'quién modificó', 'quien modifico',
  'eventos del documento', 'timeline', 'registro de cambios',
  'qué pasó', 'que paso', 'qué ocurrió', 'que ocurrio',
  'acciones realizadas', 'log', 'logs',
];

const USER_PROFILE_KEYWORDS = [
  'quién soy', 'quien soy', 'mi perfil', 'mi cuenta', 'mi plan',
  'mi nombre', 'mi email', 'mi correo', 'mi rol', 'mi usuario',
  'datos de mi cuenta', 'información de mi perfil', 'info de mi cuenta',
  'mi suscripción', 'mi suscripcion', 'plan activo', 'plan contratado',
];

const USER_USAGE_KEYWORDS = [
  'mis consumos', 'mi consumo', 'cuántos documentos he', 'cuantos documentos he',
  'cuántos documentos creé', 'cuantos documentos cree',
  'límite', 'limite', 'cuánto he usado', 'cuanto he usado',
  'documentos disponibles', 'documentos restantes', 'almacenamiento',
  'créditos', 'creditos', 'uso de ia', 'uso de ai',
  'cuántos documentos tengo', 'cuantos documentos tengo',
  'documentos este mes', 'documentos del mes',
];

const BILLING_KEYWORDS = [
  'facturación', 'facturacion', 'mi factura', 'mis facturas',
  'cuánto he consumido', 'cuanto he consumido', 'consumo este mes',
  'mi suscripción', 'mi suscripcion', 'renovación', 'renovacion',
  'costo', 'precio', 'pago', 'cobro', 'cargo',
  'plan activo', 'cambiar plan', 'cancelar plan',
];

const USER_CREATED_DOCUMENTS_KEYWORDS = [
  'documentos que creé', 'documentos que cree', 'documentos que he creado',
  'mis documentos creados', 'documentos creados por mí', 'documentos creados por mi',
  'qué documentos creé', 'que documentos cree', 'qué he creado', 'que he creado',
  'documentos míos', 'documentos mios', 'mis documentos',
];

const USER_ASSIGNED_DOCUMENTS_KEYWORDS = [
  'documentos asignados', 'documentos asignados a mí', 'documentos asignados a mi',
  'qué tengo asignado', 'que tengo asignado', 'me han asignado',
  'documentos donde participo', 'documentos en los que participo',
  'documentos compartidos conmigo', 'documentos que me enviaron',
  'documentos para firmar', 'documentos para revisar', 'documentos para aprobar',
];

const USER_PARTICIPATIONS_KEYWORDS = [
  'mi participación', 'mi participacion', 'mis participaciones',
  'dónde participo', 'donde participo', 'en qué documentos participo',
  'en que documentos participo', 'qué falta que firme', 'que falta que firme',
  'qué he firmado', 'que he firmado', 'qué he aprobado', 'que he aprobado',
  'estado de mi participación', 'estado de mi participacion',
  'como firmante', 'como revisor', 'como aprobador', 'como observador',
];

const DOCUMENT_TYPES_ASSIGNED_KEYWORDS = [
  'tipos de documentos', 'qué tipos de documentos', 'que tipos de documentos',
  'tipos de documentos asignados', 'tipos de documentos que tengo',
  'contratos asignados', 'nda asignados', 'convenios asignados',
  'qué tipos tengo', 'que tipos tengo',
];

const PENDING_TASKS_KEYWORDS = [
  'qué tengo pendiente', 'que tengo pendiente', 'mis pendientes',
  'acciones pendientes', 'tareas pendientes', 'qué debo hacer', 'que debo hacer',
  'pendiente de firma', 'pendiente de revisión', 'pendiente de revision',
  'pendiente de aprobación', 'pendiente de aprobacion',
  'qué me falta', 'que me falta', 'qué está vencido', 'que esta vencido',
  'acciones vencidas', 'documentos vencidos', 'urgente',
];

const NOTIFICATIONS_KEYWORDS = [
  'notificaciones', 'mis notificaciones', 'notificación', 'notificacion',
  'alertas', 'avisos', 'mensajes recibidos', 'qué notificaciones tengo',
  'notificaciones sin leer', 'notificaciones pendientes',
];

const CONTACTS_KEYWORDS = [
  'mis contactos', 'contacto', 'contactos', 'directorio',
  'buscar contacto', 'agregar contacto', 'lista de contactos',
  'quién está en mis contactos', 'quien esta en mis contactos',
];

const TEMPLATES_KEYWORDS = [
  'plantilla', 'plantillas', 'template', 'templates',
  'mis plantillas', 'crear plantilla', 'usar plantilla',
  'variables de plantilla', 'campos de plantilla',
];

const FORMS_KEYWORDS = [
  'formulario', 'formularios', 'form', 'forms',
  'mis formularios', 'crear formulario', 'llenar formulario',
  'campos del formulario', 'constructor de formularios',
];

const SIGNING_HELP_KEYWORDS = [
  'cómo firmar', 'como firmar', 'proceso de firma', 'firma electrónica',
  'e.firma', 'efirma', 'firma autógrafa', 'firma otp', 'otp de firma',
  'iniciar firma', 'completar firma', 'método de firma', 'metodo de firma',
  'ayuda para firmar', 'no puedo firmar', 'error al firmar',
];

const EXTERNAL_PARTICIPANT_KEYWORDS = [
  'soy participante externo', 'me invitaron a firmar', 'recibí un link',
  'recibi un link', 'portal de participante', 'acceso por token',
  'cómo accedo', 'como accedo', 'no tengo cuenta',
];

const STRUCTURED_KEYWORDS = [
  'quién', 'quien', 'creó', 'creo', 'creado', 'autor',
  'estado', 'estados', 'pendiente', 'pendientes',
  'firmado', 'firmados', 'firmante', 'firmantes', 'firma', 'firmar', 'firmas',
  'vence', 'vencen', 'vencimiento', 'vencido', 'vencidos', 'vencer',
  'revisión', 'revision', 'revisar', 'en revisión',
  'participante', 'participantes', 'participación',
  'expediente', 'carpeta', 'carpetas',
  'tarea', 'tareas', 'pendiente de',
  'muéstrame', 'muestrame', 'lista', 'listar', 'mostrar',
  'cuántos', 'cuantos', 'cuándo', 'cuando', 'fecha',
  'responsable', 'asignado', 'creador',
  'cancelado', 'cancelados', 'borrador', 'completado',
];

const RAG_KEYWORDS = [
  'cláusula', 'clausula', 'cláusulas', 'clausulas',
  'resumen', 'resume', 'resumir', 'resumé',
  'riesgo', 'riesgos', 'legal', 'legales',
  'obligación', 'obligaciones', 'obligacion',
  'vigencia', 'vigente', 'duración', 'duracion',
  'penalización', 'penalizacion', 'penalidad',
  'contenido', 'dice', 'establece', 'estipula',
  'contrato', 'acuerdo', 'convenio',
  'checklist', 'cumplimiento', 'compliance',
  'análisis', 'analisis', 'analiza', 'analizar',
  'extrae', 'extraer', 'extracción',
  'qué dice', 'que dice', 'qué establece',
];

const CONFIGURATION_SECURITY_KEYWORDS = [
  'configuración', 'configuracion', 'seguridad', 'workspace', 'ajustes',
  '2fa', 'mfa', 'autenticación de dos factores', 'autenticacion de dos factores',
  'sesiones activas', 'dispositivos', 'permisos', 'roles del workspace',
  'notificaciones de workspace', 'logo del workspace', 'nombre del workspace',
];

const REPORTS_ANALYSIS_KEYWORDS = [
  'reporte', 'reportes', 'informe', 'informes', 'estadísticas', 'estadisticas',
  'métricas', 'metricas', 'exportar reporte', 'generar reporte',
  'actividad del workspace', 'uso del sistema', 'análisis de uso', 'analisis de uso',
];
...
export function classifyIntent(question: string, currentRoute?: string): IntentResult {
  ...
  const hasUserCreatedDocs = USER_CREATED_DOCUMENTS_KEYWORDS.some(kw => q.includes(normalize(kw)));
  const hasUserAssignedDocs = USER_ASSIGNED_DOCUMENTS_KEYWORDS.some(kw => q.includes(normalize(kw)));
  const hasUserParticipations = USER_PARTICIPATIONS_KEYWORDS.some(kw => q.includes(normalize(kw)));
  const hasDocTypesAssigned = DOCUMENT_TYPES_ASSIGNED_KEYWORDS.some(kw => q.includes(normalize(kw)));
  ...
  const hasExternalParticipant = EXTERNAL_PARTICIPANT_KEYWORDS.some(kw => q.includes(normalize(kw)));
  ...
  if (scope === 'auth' || scope === 'registration' || scope === 'email_verification' || scope === 'password_recovery') {
    intent = 'general_help';
  ...
  if (hasSensitiveProfile) {
    ...
    q.includes('riesgo') || q.includes('analiz') || q.includes('legal') ||
    q.includes('clausula') || q.includes('obligacion') || q.includes('penaliz')
  ) {
    intent = 'legal_analysis';
  } else if (q.includes('cumplimiento') || q.includes('compliance') || q.includes('checklist')) {
    intent = 'compliance_analysis';
  } else if (q.includes('generar') || q.includes('genera') || q.includes('redactar') || q.includes('crear contrato')) {
    intent = 'contract_generation';
  } else if (q.includes('firma') || q.includes('firmar') || q.includes('firmante') || q.includes('firmo')) {
    intent = 'signature_status';
  } else if (q.includes('expediente') || q.includes('carpeta')) {
    intent = 'expediente_search';
  } else if (
    q.includes('estado') || q.includes('borrador') || q.includes('completado') ||
    q.includes('cancelado') || q.includes('revision') || q.includes('vencido')
  ) {
    intent = 'document_status_search';
  } else if (
    q.includes('quien') || q.includes('autor') || q.includes('creo') ||
    q.includes('creado') || q.includes('participante')
  ) {
    intent = 'document_status_search';
  } else if (hasRAG) {
