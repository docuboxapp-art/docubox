      ...
      >
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button
            ...
            onClick={() => {
              onSave({
                customName: customName.trim() || field.label,
                showLabelInDocument: showLabel,
              });
              onClose();
            }}
...
function FieldTypeConfigModal({
  ...
  const handleSave = () => {
    ...
    if (isNumero) {
      ...
      result.numberFormat = numberFormat as any;
    }
    if (isMoneda) {
      result.currency = currency as any;
      result.currencySymbol = currencySymbol;
    }
    if (isFecha) result.dateFormat = dateFormat as any;
    if (isHora) {
      result.timeFormat = timeFormat as any;
          ...
          {isImagen && (
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary bg-white"
              >
...
function DraggableField({
  ...
  colorHex?: string;
  placementKind?: PlacedField['placementKind'];
  ...
  return (
    ...
    >
      ...
      {disabled ? (
        ...
        <span
          className="text-gray-300 text-base leading-none shrink-0 select-none"
...
function PlacedFieldWidget({
  ...
  onUpdateFieldConfig?: (
    id: string,
    config: { customName: string; showLabelInDocument: boolean }
  ) => void;
  ...
  const cryptographicPreview: Record<CryptographicElementType, { eyebrow: string; text: string }> =
    {
      ...
      document_seal: {
        ...
        text: 'SDL-DBX-... · SHA-256 · RSA-PSS-SHA256 · Se genera al completar',
      },
      timestamp: {
        eyebrow: 'ESTAMPA DE TIEMPO',
        text: 'RFC 3161 · TSA verificada · Fecha UTC al completar',
      },
      evidence_chain: {
        eyebrow: 'CADENA DE EVIDENCIA',
        text: '||DOCUBOX_EVIDENCE|1.0| ... Se genera al completar ... ||',
      },
    };
  const cryptoPreview = field.cryptographicType
    ? cryptographicPreview[field.cryptographicType]
    : null;
  ...
  return (
    <>
      ...
      >
        ...
        {selected && (
          ...
          >
            ...
            {!isCasilla &&
              field.label !== 'Imagen' &&
              (hasTypeConfigOption || isDropdown || isRadio) && (
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
      ...
      {/* Field Type Config Modal */}
      {showTypeConfigModal && (
        <FieldTypeConfigModal
          field={field}
          onSave={(cfg) => {
            onUpdateFieldTypeConfig?.(field.id, cfg);
          }}
          onClose={() => setShowTypeConfigModal(false)}
        />
      )}
    </>
  );
...
function getPasswordStrength(code: string): {
...
function SecurityTab({ documentoId }: { documentoId: string }) {
  ...
  const writeAuditLog = useCallback(
    async (action: string, uid?: string, details?: Record<string, unknown>) => {
  ...
  const saveToDb = useCallback(
    async (s: SecuritySettings, auditAction?: string) => {
      setSaving(true);
      setSaveError(null);
      try {
  ...
  return (
    <div className="space-y-4">
      ...
      {(saving || savedOk || saveError) && (
        <div
          className={`flex items-center gap-2 text-xs px-3 py-2 rounded-lg ${saving ? 'bg-blue-50 text-blue-600' : savedOk ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}
        >
          {saving && (
            <>
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        <label className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-gray-50 transition-colors">
          <input
            ...
            onChange={(e) => {
              update(
                { vencimientoEnabled: e.target.checked },
                e.target.checked ? 'Protección habilitada' : undefined
              );
              if (!e.target.checked) {
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onBlur={() => {
                    setFechaTouched(true);
                    setFechaError(validateFechaVencimiento(security.fechaVencimiento));
                  }}
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  onBlur={() => {
                    setCodigoTouched(true);
                    setCodigoError(validateCodigoAcceso(security.codigoAcceso));
                  }}
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        <label className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-gray-50 transition-colors">
          <input
            ...
            onChange={(e) =>
              update(
                { proteccionAdicionalEnabled: e.target.checked },
                e.target.checked ? 'Protección habilitada' : undefined
              )
            }
            className="w-4 h-4 rounded accent-primary cursor-pointer"
          ...
          <div className="border-t border-gray-100 px-4 pb-4 pt-3 bg-white space-y-3">
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5 flex gap-2">
              ...
              <p className="text-xs text-amber-700">
                <span className="font-semibold">¡Atención!</span> Esta acción es irreversible una
                vez que se envíe el documento.
              </p>
            </div>
            {[
      ...
      {/* Legal Hold is a lifecycle control: it cannot be toggled as a document setting. */}
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3.5">
          <span className="text-sm font-semibold text-primary flex-1">Protección Legal Hold</span>
          <Lock size={14} className="text-gray-400 shrink-0" />
        </div>
        <div className="border-t border-gray-100 px-4 pb-3 pt-2 bg-white">
          <p className="text-xs text-gray-500">
            Se solicita después de crear el documento, con motivo y trazabilidad. No puede activarse
            ni liberarse mediante una casilla.
          </p>
        </div>
      </div>
    </div>
...
export function StepAjustes({
  ...
  const displayParticipants =
    participants.length > 0
      ? participants
      : [{ id: 'default-1', name: 'LUIS ALBERTO HERNÁNDEZ', email: '', role: 'firmante' as const }];

  useEffect(() => {
    if (displayParticipants.length > 0 && !selectedParticipantId)
      setSelectedParticipantId(displayParticipants[0].id);
  ...
  useEffect(() => {
    if (!cryptoPlacementEnabled) {
      setPlacedFields((current) =>
        current.filter((field) => field.placementKind !== 'cryptographic')
      );
    }
  ...
  useEffect(() => {
    const firmanteParticipants = displayParticipants.filter(
      (p) => (p.acto || '').toLowerCase() === 'firmante' || participants.length === 0
    );
    const hasFirmaForAll =
      firmanteParticipants.length === 0 ||
      firmanteParticipants.every((p) =>
    ...
    const allAreFirmantes =
      participants.length > 0 &&
      participants.every((p) => (p.acto || '').toLowerCase() === 'firmante');
    onFixarCamposChange?.(
      fixarCampos || !!isCondicional,
      allAreFirmantes ? hasFirmaForAll : hasFirma
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fixarCampos, placedFields, isCondicional, participants]);

  // Save placed fields to Supabase whenever they change
  const saveFieldsToSupabase = useCallback(
  ...
  const selectedParticipant =
    displayParticipants.find((p) => p.id === selectedParticipantId) || displayParticipants[0];
  const selectedParticipantIdx = displayParticipants.findIndex(
  ...
  const cryptographicFields: Array<{
    icon: React.ReactNode;
    label: string;
  ...
  const handleClickPlace = (label: string, required?: boolean) => {
    ...
    if (label === 'Firma') {
      ...
      const alreadyHasFirma = placedFields.some(
        (f) => f.label === 'Firma' && f.participantId === participantId
      );
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      if (data.label === 'Firma') {
        const alreadyHasFirma = placedFields.some(
          (f) => f.label === 'Firma' && f.participantId === data.participantId
        );
        if (alreadyHasFirma) return;
      }
    ...
    <div className="w-full flex gap-6">
      ...
      <div className="w-[38%] flex flex-col gap-4 overflow-y-auto max-h-[calc(100vh-220px)]">
        ...
        <div className="rounded-lg border border-slate-200/90 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
          ...
          <label
            className={`flex items-start gap-3 rounded-lg border border-slate-200 bg-slate-50/70 p-3.5 transition-colors ${isCondicional ? 'cursor-not-allowed opacity-80' : 'cursor-pointer hover:border-slate-300 hover:bg-slate-50'}`}
          >
            <input
              type="checkbox"
              checked={isCondicional ? true : fixarCampos}
              onChange={(e) => {
                if (!isCondicional) {
                  if (!e.target.checked && standardPlacedFields.length > 0) {
                    setShowDeleteConfirm(true);
                  } else {
                    setFixarCampos(e.target.checked);
                  }
                }
              }}
              disabled={isCondicional}
              className="mt-0.5 h-4 w-4 shrink-0 rounded accent-primary cursor-pointer disabled:cursor-not-allowed"
            />
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-600 text-slate-800">
                Asignar campos a participantes
              </span>
            ...
            <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3 flex gap-2">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs leading-5 text-blue-600">
                  La asignación de campos es obligatoria para asegurar el funcionamiento correcto
        ...
        {(fixarCampos || isCondicional) && (
          <div className="rounded-lg border border-slate-200/90 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
            ...
            <div className="relative mb-4">
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-800 flex-1 truncate">
                  ...
                  participants.length > 0 &&
                  !selectedParticipant?.acto
                    ? ' (tú)'
                    : ''}
                </span>
                <ChevronDown
                  size={14}
                  className={`text-gray-400 transition-transform ${participantDropdownOpen ? 'rotate-180' : ''}`}
                />
              </button>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden mb-2">
              <button
                type="button"
                onClick={() => setCamposParticipanteOpen((v) => !v)}
                className="w-full flex items-center gap-1.5 px-4 py-3.5 text-left hover:bg-gray-50 transition-colors"
              >
                <div className="flex items-center gap-2 w-full">
                  <span className="flex-1 text-sm font-600 text-gray-800">
                    Campos del participante
                  </span>
                  <ChevronDown
                    size={16}
                    className={`text-gray-400 transition-transform ${camposParticipanteOpen ? 'rotate-180' : ''}`}
                  />
                </div>
              </button>
              {camposParticipanteOpen && (
                <div className="border-t border-gray-200 px-4 pb-4 space-y-2 pt-2">
                  {participantFields.map((field) => (
                    <DraggableField
                      key={`${selectedParticipant?.id}-${field.label}`}
                      icon={field.icon}
                      label={field.label}
                      ...
                      participantName={selectedParticipant?.name}
                      colorHex={selectedColorHex}
                      onClickPlace={() => handleClickPlace(field.label, field.required)}
                      disabled={
                        field.label === 'Firma' &&
                        placedFields.some(
                          (f) => f.label === 'Firma' && f.participantId === selectedParticipant?.id
                        )
                      }
                    />
                  ))}
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <button
                type="button"
                onClick={() => setCamposGeneralesOpen((v) => !v)}
                className="w-full flex items-center gap-1.5 px-4 py-3.5 text-left hover:bg-gray-50 transition-colors"
              >
                <div className="flex items-center gap-2 w-full">
                  ...
                  <ChevronDown
                    size={16}
                    className={`text-gray-400 transition-transform ${camposGeneralesOpen ? 'rotate-180' : ''}`}
                  />
                </div>
              </button>
              {camposGeneralesOpen && (
                <div className="border-t border-gray-200 px-4 pb-4 space-y-2 pt-2">
                  {generalFields.map((field) => (
                    <DraggableField
                      key={`${selectedParticipant?.id}-general-${field.label}`}
                      icon={field.icon}
                      label={field.label}
                      participantId={selectedParticipant?.id}
                      participantName={selectedParticipant?.name}
                      colorHex={selectedColorHex}
                      onClickPlace={() => handleClickPlace(field.label)}
                    />
                  ))}
                </div>
              )}
                  ...
                  {standardPlacedFields.map((field) => (
                    ...
                    >
                      <div
                        ...
                        style={{ backgroundColor: field.colorHex || '#9ca3af' }}
                      />
                      ...
                      {field.participantName && (
                        <span className="text-gray-400 truncate max-w-[60px]">
                          {field.participantName.split(' ')[0]}
                        </span>
                      )}
                      <button
                        onClick={() =>
                          setPlacedFields((prev) => prev.filter((x) => x.id !== field.id))
                        }
        ...
        {cryptoPlacementEnabled && (
          <div className="rounded-lg border border-blue-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
            ...
            <div className="overflow-hidden rounded-lg border border-slate-200">
              <button
                type="button"
                onClick={() => setSellosCadenaOpen((value) => !value)}
                className="flex w-full items-center gap-2 px-4 py-3.5 text-left transition-colors hover:bg-slate-50"
              >
                <span className="flex-1 text-sm font-600 text-slate-800">
                  Elementos disponibles
                </span>
                <ChevronDown
                  size={16}
                  className={`text-slate-400 transition-transform ${sellosCadenaOpen ? 'rotate-180' : ''}`}
                />
              </button>
              {sellosCadenaOpen && (
                <div className="space-y-2 border-t border-slate-200 px-4 pb-4 pt-3">
                  {cryptographicFields.map((field) => {
                    const alreadyPlaced = placedFields.some(
                      (placed) => placed.cryptographicType === field.type
                    );
                    return (
                      <DraggableField
                        key={field.type}
            ...
            {placedFields.some((field) => field.placementKind === 'cryptographic') && (
              <div className="mt-4 border-t border-slate-100 pt-4">
                ...
                <div className="space-y-1.5">
                  {placedFields
                    .filter((field) => field.placementKind === 'cryptographic')
                    .map((field) => (
                      <div
                        key={field.id}
                        className="flex items-center gap-2 rounded-lg bg-blue-50/70 px-3 py-2 text-xs text-slate-600"
                      >
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setPlacedFields((prev) => prev.filter((item) => item.id !== field.id))
                          }
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col h-full overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Reducir zoom"
                className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                <ZoomOut size={14} />
              </button>
              <button
                ...
                className="min-w-[48px] h-8 px-2 flex items-center justify-center rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors"
              >
                {displayZoom}%
              </button>
              <button
                onClick={() => {
                  const next = Math.min(200, zoomLevel + 25);
                  setZoomLevel(next);
                  setDisplayZoom(next);
                }}
          ...
          >
            ...
            <div
              ref={docSheetRef}
              data-doc-sheet="true"
              className="relative bg-white shadow-lg rounded origin-top shrink-0"
              style={{
                width: `${zoomLevel}%`,
            ...
            >
              {file && isPdf ? (
                pdfLoading ? (
                  ...
                  <iframe
                    src={`${pdfObjectUrl}#page=${currentPage}&toolbar=0&navpanes=0&scrollbar=0`}
                ...
                <div className="w-full h-full flex items-center justify-center bg-gray-800">
                  <div className="text-gray-400 text-sm">No hay documento cargado</div>
                </div>
              ) : (
                <div className="w-full h-full flex items-center justify-center bg-gray-800">
                  <div className="text-gray-400 text-sm">No hay documento cargado</div>
              ...
              {placedFields
                ...
                .map((field) => (
                  <PlacedFieldWidget
                    key={field.id}
                    field={field}
                    onRemove={(id) => setPlacedFields((prev) => prev.filter((x) => x.id !== id))}
                    onMove={(id, x, y) =>
                      setPlacedFields((prev) => prev.map((f) => (f.id === id ? { ...f, x, y } : f)))
                    }
                    onResize={(id, width, height, x, y) =>
                      setPlacedFields((prev) =>
                        prev.map((f) => (f.id === id ? { ...f, width, height, x, y } : f))
                      )
                    }
                    onUpdateOptions={(id, opts) =>
                      setPlacedFields((prev) =>
                        prev.map((f) => (f.id === id ? { ...f, dropdownOptions: opts } : f))
                      )
                    }
                    onUpdateRadioOptions={(id, opts) =>
                      setPlacedFields((prev) =>
                        prev.map((f) => (f.id === id ? { ...f, radioOptions: opts } : f))
                      )
                    }
                    onUpdateCasillaLabel={(id, lbl) =>
                      setPlacedFields((prev) =>
                        prev.map((f) => (f.id === id ? { ...f, casillaLabel: lbl } : f))
                      )
                    }
                    onUpdateFieldConfig={(id, cfg) =>
                      setPlacedFields((prev) =>
                        prev.map((f) => (f.id === id ? { ...f, fieldConfig: cfg } : f))
                      )
                    }
                    onUpdateFieldTypeConfig={(id, cfg) =>
                      setPlacedFields((prev) =>
                        prev.map((f) => (f.id === id ? { ...f, fieldTypeConfig: cfg } : f))
                      )
                    }
                    userData={
