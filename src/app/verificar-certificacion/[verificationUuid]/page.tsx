        ...
        {result && (
          <>
            <section className="mt-6 flex flex-col gap-5 border-b border-slate-200 pb-7 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="rounded-md border border-slate-200 bg-white px-4 py-3 text-xs text-slate-500"><p className="font-600 text-slate-900">UUID de verificacion</p><p className="mt-1 font-mono">{result.verification_uuid}</p></div>
            </section>
            ...
            <section className="mt-5 overflow-hidden rounded-md border border-slate-200 bg-white">
              <header className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-md bg-blue-50 text-blue-600"><Fingerprint size={18} /></span><div><h2 className="text-sm font-600">Sello Digital Docubox</h2><p className="mt-0.5 text-xs text-slate-500">Proteccion criptografica de la Cadena Original Docubox</p></div></div>
              ...
              <div className="grid gap-0 lg:grid-cols-[1fr_0.9fr]">
                ...
                <div className="p-5"><p className="text-xs font-600 uppercase text-slate-500">Sello Base64 abreviado</p><pre className="mt-3 max-h-44 overflow-auto whitespace-pre-wrap break-all rounded-md border border-slate-200 bg-slate-50 p-4 font-mono text-[11px] leading-5 text-slate-600">{result.document_seal.seal_base64_preview}</pre><p className="mt-3 text-xs leading-5 text-slate-500">La validacion utiliza siempre el sello completo. Esta abreviacion es solamente visual.</p></div>
              </div>
              <div className="border-t border-slate-200 px-5 py-4"><p className="text-xs font-600 uppercase text-slate-500">Artefactos de verificacion</p><div className="mt-3 flex flex-wrap gap-2">{result.document_seal.downloads.map((name) => <a key={name} href={`/api/verify/${encodeURIComponent(result.verification_uuid)}/artifacts/${encodeURIComponent(name)}`} className="inline-flex h-9 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs font-600 text-slate-700 transition hover:border-blue-300 hover:text-blue-700"><Download size={14} />{name}</a>)}</div></div>
            </section>
            <section className="mt-5 rounded-md border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><Hash size={17} className="text-blue-600" /><h2 className="text-sm font-600">Raiz de certificacion SHA-256</h2></div><p className="mt-3 break-all font-mono text-xs leading-6 text-slate-600">{result.certification_root_sha256}</p></section>
          </>
        )}
      </main>
    </div>
...
function VerificationBadge({ status }: { status: string }) {
  if (status === 'VALID') return <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-600 text-emerald-700"><CheckCircle2 size={14} />Validacion exitosa</span>;
  if (status === 'REVOKED') return <span className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1 text-xs font-600 text-amber-700"><AlertCircle size={14} />Llave revocada</span>;
  return <span className="inline-flex items-center gap-2 rounded-full bg-red-50 px-3 py-1 text-xs font-600 text-red-700"><AlertCircle size={14} />Validacion fallida</span>;
}
...
function SealBadge({ status }: { status: VerificationResult['document_seal']['status'] }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-600 ${valid ? 'bg-emerald-50 text-emerald-700' : status === 'REVOKED' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>{valid ? <CheckCircle2 size={14} /> : <AlertCircle size={14} />}{labels[status]}</span>;
}

function SealValue({ label, value, mono = false }: { label: string; value: string | null; mono?: boolean }) {
  return <div><p className="text-[11px] font-600 uppercase text-slate-500">{label}</p><p className={`mt-1 break-all text-sm text-slate-900 ${mono ? 'font-mono text-xs leading-5' : ''}`}>{value || 'No disponible'}</p></div>;
}

function ValidationCard({ icon: Icon, title, rows }: { icon: React.ElementType; title: string; rows: Array<[string, boolean]> }) {
  return <div className="rounded-md border border-slate-200 bg-white"><header className="flex items-center gap-2 border-b border-slate-200 px-5 py-4"><Icon size={17} className="text-blue-600" /><h2 className="text-sm font-600">{title}</h2></header><div className="divide-y divide-slate-100">{rows.map(([label, valid]) => <div key={label} className="flex items-center justify-between gap-3 px-5 py-3 text-sm"><span className="text-slate-600">{label}</span><span className={`inline-flex items-center gap-1 text-xs font-600 ${valid ? 'text-emerald-700' : 'text-red-600'}`}>{valid ? <CheckCircle2 size={14} /> : <AlertCircle size={14} />}{valid ? 'Valido' : 'Invalido'}</span></div>)}</div></div>;
}
