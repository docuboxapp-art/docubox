...
const navItems = [
  { href: '/notificaciones-certificadas', label: 'Notificaciones', icon: MailCheck },
  { href: '/notificaciones-certificadas/nueva', label: 'Nueva notificacion', icon: Plus },
  { href: '/notificaciones-certificadas/constancias', label: 'Constancias', icon: BadgeCheck },
  { href: '/notificaciones-certificadas/auditoria', label: 'Auditoria', icon: ScrollText },
];
    ...
    <div className="-mx-4 -my-4 min-h-[calc(100vh-4rem)] bg-[#f6f8fb] dark:bg-background md:-my-6">
      <div className="mx-auto grid w-full max-w-[1680px] lg:grid-cols-[224px_minmax(0,1fr)]">
        <aside className="hidden min-h-[calc(100vh-4rem)] border-r border-slate-200 bg-white px-3 py-5 dark:border-border dark:bg-card lg:block">
          <p className="px-3 pb-3 text-[11px] font-600 uppercase tracking-[0.08em] text-slate-400">
            Docubox Notifica
          </p>
          <nav className="space-y-1">
            {navItems.map((item) => {
              const active =
                item.href === '/notificaciones-certificadas'
                  ? pathname === item.href
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex h-10 items-center gap-3 rounded-md px-3 text-sm transition ${active ? 'bg-blue-50 font-600 text-primary dark:bg-primary/10' : 'font-500 text-slate-600 hover:bg-slate-50 hover:text-slate-950 dark:text-muted-foreground dark:hover:bg-muted'}`}
                >
                  <Icon size={17} />
                  {item.label}
                </Link>
          ...
          <div className="mx-3 mt-6 border-t border-slate-200 pt-5 dark:border-border">
            <div className="flex items-start gap-2.5 text-xs leading-5 text-slate-500 dark:text-muted-foreground">
              <FileClock size={15} className="mt-0.5 shrink-0 text-cyan-700" />
              Cada comunicacion conserva su documento canonico y una linea de evidencia
              independiente.
            </div>
          </div>
        </aside>
...
export function NotificaHeader({
  title,
  description,
  action,
}: {
...
export function NotificationStatusBadge({ status }: { status: NotificationStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-600 ${toneClasses[meta.tone]}`}
    >
      {meta.label}
    </span>
  );
...
export function EvidenceBadge({ level }: { level: EvidenceLevel }) {
  return (
    <span
      title={EVIDENCE_META[level].description}
      className="inline-flex items-center gap-1.5 rounded-full border border-cyan-200 bg-cyan-50 px-2.5 py-1 text-xs font-600 text-cyan-800 dark:border-cyan-900 dark:bg-cyan-950/30 dark:text-cyan-300"
    >
      <span className="h-1.5 w-1.5 rounded-full bg-cyan-600" />
      {level} · {EVIDENCE_META[level].label}
    </span>
  );
}
...
export function Metric({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-border dark:bg-card">
      <div
        className={`inline-flex min-w-9 items-center justify-center rounded-md px-2 py-1 text-xl font-600 ${colors[tone]}`}
      >
        {value}
      </div>
      <p className="mt-3 text-sm font-600 text-slate-950 dark:text-foreground">{label}</p>
      <p className="mt-1 text-xs text-slate-500 dark:text-muted-foreground">{detail}</p>
    </div>
  );
}
