...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ payload: { tipo: string; count: number } }> }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-border rounded-xl shadow-dropdown p-3">
        ...
        <p className="text-sm font-700 text-primary tabular-nums mt-0.5">{payload[0].payload.count} docs</p>
      </div>
    );
  }
  return null;
...
export default function DocumentTypeChart() {
  return (
    <div className="bg-white rounded-xl border border-border shadow-card p-5 h-full">
      <div className="mb-4">
        <h2 className="text-base font-600 text-foreground">Por Tipo de Documento</h2>
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={typeData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(214, 20%, 92%)" horizontal vertical={false} />
          <XAxis
            dataKey="tipo"
