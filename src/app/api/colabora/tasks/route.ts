...
const taskSchema = z.object({
  ...
  tipo: z.enum(['firmar_documento','revisar_documento','aprobar_documento','subir_anexo','validar_identidad','corregir_datos','resolver_comentario','confirmar_lectura','descargar_constancia','validar_efirma','obtener_nom151','cerrar_expediente']).default('revisar_documento'),
...
export async function GET(request: Request) {
  try {
    ...
    let query = service.from('tareas')
      .select('id,title,description,tipo,prioridad,estado,riesgo,due_date,sla,sla_due_at,is_overdue,is_blocked,blocked_reason,assigned_to,assigned_team_id,responsible_name,creator_name,document_name,document_id,expediente_id,expediente_name,tags,collaboration_space_id,confidentiality,optimistic_version,created_at,updated_at')
    ...
    if (search) query = query.or(`title.ilike.%${search.replace(/[%_,]/g, '')}%,description.ilike.%${search.replace(/[%_,]/g, '')}%`);
...
export async function POST(request: Request) {
  try {
    ...
    const { service, user } = await authorizeCollaborationRequest(request, input.workspace_id, 'tasks.create', true);
    const [profile, assignee] = await Promise.all([
      service.from('user_profiles').select('full_name').eq('id', user.id).maybeSingle(),
      input.assigned_to ? service.from('user_profiles').select('full_name,email').eq('id', input.assigned_to).maybeSingle() : Promise.resolve({ data: null, error: null }),
    ]);
    if (profile.error) throw profile.error;
    if (assignee.error) throw assignee.error;
    const result = await service.from('tareas').insert({
      ...input,
