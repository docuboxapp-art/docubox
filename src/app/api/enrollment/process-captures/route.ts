...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !token ||
      !tipoId ||
      !validImageBase64(anversoData) ||
      !validImageBase64(reversoData) ||
      !validImageBase64(selfieData)
    ) {
      return NextResponse.json(
        { error: 'Las capturas requeridas no son validas.' },
        { status: 400 }
      );
    }
    ...
    if (!key) {
      return NextResponse.json(
        { error: 'El cifrado de enrolamiento no esta configurado.' },
        { status: 503 }
      );
    }
    ...
    if (!providerResponse.ok) {
      return NextResponse.json(
        { error: 'El proveedor no pudo procesar las capturas.' },
        { status: 502 }
      );
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'No fue posible conservar las capturas cifradas.' },
        { status: 500 }
      );
    }
