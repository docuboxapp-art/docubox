  ...
  try {
    ...
    if (tokenData.processing_status !== 'validated') {
      return NextResponse.json(
        {
          error:
            'La identidad y la prueba de vida deben validarse antes de completar el enrolamiento.',
    ...
    const tipoIdentificacion =
      textField(fields, 'tipo_identificacion', 'document_type') ||
    ...
    const { data: resultData, error: resultError } = await supabase
      .rpc('complete_identity_enrollment', {
        p_token: tokenHash,
        p_result: {
          nombre,
    ...
    if (resultError || !resultData) {
      ...
      return NextResponse.json(
        { error: 'No fue posible registrar el resultado del enrolamiento.' },
        { status: 500 }
      );
    }
    return NextResponse.json({
      success: true,
      enrollmentResultId: resultData.enrollment_result_id,
    });
  } catch (error) {
    console.error('[complete] Failed:', error instanceof Error ? error.message : 'unknown');
    return NextResponse.json(
      { error: 'No fue posible completar el enrolamiento.' },
      { status: 500 }
    );
  }
}
