...
} from '@/lib/organization/webhook-security';

export const runtime = 'nodejs';

const safeColumns =
  'id,workspace_id,name,endpoint_url,environment,event_types,status,failure_count,last_delivery_at,last_success_at,last_failure_at,created_by,created_at,updated_at';
...
function normalizeEvents(value: unknown) {
  ...
  return [
    ...new Set(
      value.filter(
        (event): event is string => typeof event === 'string' && supportedEvents.has(event)
      )
    ),
  ];
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !endpointUrl || !eventTypes.length)
      return Response.json(
        { success: false, error: 'Completa nombre, URL HTTPS y eventos.' },
        { status: 400 }
      );
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!id || !['enable', 'disable', 'revoke', 'rotate'].includes(action))
    ...
    if (readError || !existing)
      return Response.json(
        { success: false, error: 'No se encontró el webhook.' },
        { status: 404 }
      );
    ...
    if (action === 'rotate') {
      ...
      Object.assign(changes, {
        ...
        secret_iv: encrypted.iv,
        secret_tag: encrypted.tag,
        secret_version: Number(existing.secret_version || 1) + 1,
      });
    ...
    await service.from('organization_audit_events').insert({
      workspace_id: workspaceId,
      actor_user_id: user.id,
      event_type: `integration.webhook.${action}`,
      resource_type: 'organization_webhook_endpoint',
      resource_id: id,
      summary: `Webhook ${action}: ${existing.name}`,
      severity: action === 'revoke' ? 'critical' : 'high',
      module: 'integrations',
    });
    return Response.json({
      success: true,
      data,
      ...(secretValue ? { secret: secretValue, shown_once: true } : {}),
    });
  } catch (cause) {
    return organizationApiFailure(cause);
  }
