...
const safeColumns =
  'id,workspace_id,name,environment,key_prefix,scopes,status,last_used_at,expires_at,rotated_at,created_by,created_at,revoked_at';
...
function normalizeScopes(value: unknown) {
  ...
  return [
    ...new Set(
      value.filter(
        (scope): scope is string => typeof scope === 'string' && allowedScopes.has(scope)
      )
    ),
  ];
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!name || name.length > 100)
      return Response.json({ success: false, error: 'Indica un nombre válido.' }, { status: 400 });
    if (!scopes.length)
      return Response.json(
        { success: false, error: 'Selecciona al menos un alcance.' },
        { status: 400 }
      );
    ...
    if (expiresAt && Number.isNaN(expiresAt.getTime()))
      return Response.json(
        { success: false, error: 'La fecha de expiración no es válida.' },
        { status: 400 }
      );
    ...
    return Response.json(
      { success: true, data, secret: secret.value, shown_once: true },
      { status: 201 }
    );
  } catch (cause) {
    return organizationApiFailure(cause);
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!id || !['revoke', 'rotate'].includes(action))
    ...
    if (readError || !existing)
      return Response.json(
        { success: false, error: 'No se encontró la credencial.' },
        { status: 404 }
      );
    ...
    if (action === 'revoke') {
      ...
      await service.from('organization_audit_events').insert({
        workspace_id: workspaceId,
        actor_user_id: user.id,
        event_type: 'integration.api_key.revoked',
        resource_type: 'organization_api_credential',
        resource_id: id,
        summary: `API key revocada: ${existing.name}`,
        severity: 'critical',
        module: 'integrations',
      });
      return Response.json({ success: true, data });
    ...
    const secret = createOpaqueSecret(
      existing.environment === 'production' ? 'dbx_live' : 'dbx_test'
    );
    const { data, error } = await service
      .from('organization_api_credentials')
      .update({
    ...
    await service.from('organization_audit_events').insert({
      workspace_id: workspaceId,
      actor_user_id: user.id,
      event_type: 'integration.api_key.rotated',
      resource_type: 'organization_api_credential',
      resource_id: id,
      summary: `API key rotada: ${existing.name}`,
      severity: 'high',
      module: 'integrations',
      payload: { key_prefix: secret.publicPrefix },
    });
