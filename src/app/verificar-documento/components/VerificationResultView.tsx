      ...
      <div className="flex flex-col gap-5 px-5 py-6 sm:flex-row sm:items-start sm:justify-between sm:px-7">
        <div className="flex min-w-0 items-start gap-4">
          ...
          <div className="min-w-0">
            <p className="text-xs font-600 uppercase text-slate-500">Verificación integral</p>
            <h2 className="mt-1 text-xl font-650 text-slate-950 sm:text-2xl">{result.headline}</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{result.message}</p>
          </div>
        </div>
        <span
          className={`inline-flex h-8 shrink-0 items-center rounded-full px-3 text-xs font-600 ${tone.badge}`}
        >
...
function SummaryStrip({ result }: { result: PublicVerificationResult }) {
  ...
  return (
    <dl className="grid overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)] sm:grid-cols-3">
    ...
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      <dl>
        {rows.map(([label, value], index) => (
          <div
            key={label}
            className={`grid gap-1 px-5 py-3 text-sm sm:grid-cols-[190px_minmax(0,1fr)] ${index ? 'border-t border-slate-200' : ''}`}
          >
            <dt className="text-xs font-600 uppercase text-slate-500">{label}</dt>
            <dd
              className={`${label === 'SHA-256' || label === 'Identificador' ? 'break-all font-mono text-xs' : ''} text-slate-950`}
            >
              {value}
            </dd>
          </div>
      ...
      {participants.length ? (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1080px] text-left text-sm">
            <thead className="border-b border-slate-200 bg-slate-50 text-[10px] font-600 uppercase text-slate-500">
              <tr>
                ...
                <th className="px-4 py-3">Huella de firma</th>
            ...
            <tbody>
              {participants.map((participant, index) => (
                ...
                >
                  <td className="px-5 py-4 font-650 text-slate-950">{participant.name}</td>
                  ...
                  <td className="px-4 py-4 text-slate-600">{formatUtc(participant.signedAt)}</td>
                  <td className="px-4 py-4">
                    {participant.signatureHash ? (
                      <code
                        title={participant.signatureHash}
                        className="block max-w-[220px] break-all font-mono text-[10px] leading-4 text-slate-600"
                      >
                        {participant.signatureHash}
                      </code>
                    ) : (
...
function TimestampCard({ certification }: { certification: PublicCertificationDetails | null }) {
  ...
  return (
    <section
      className={`overflow-hidden rounded-lg border bg-white ${timestamp ? 'border-primary/20' : 'border-slate-200'}`}
    >
      <div className={`${timestamp ? 'bg-primary/5' : 'bg-slate-50'} px-5 py-5 sm:px-7`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="text-lg font-650 text-slate-950">Estampa de tiempo</h3>
          <span
            className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-2 text-xs font-600 ${tone.badge}`}
          >
            <tone.Icon size={16} />
...
function TechnicalAnnex({ certification }: { certification: PublicCertificationDetails | null }) {
  ...
  return (
    <section>
      <div className="mb-4">
        <h3 className="text-xl font-650 text-slate-950">Anexo técnico de validación</h3>
...
function DocumentAccess({ result }: { result: PublicVerificationResult }) {
  ...
  if (!document.isPublic || !document.documentUrl) {
    return (
      <section className="rounded-lg border border-slate-200 bg-white px-5 py-6 sm:px-7">
        <div className="flex items-start gap-4">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-slate-100 text-slate-500">
            <LockKeyhole size={20} />
          </span>
          <div>
            <h3 className="font-650 text-slate-950">Documento privado</h3>
            <p className="mt-1 text-sm leading-6 text-slate-600">
    ...
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-650 text-slate-950">Documento final público</h3>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-slate-200 px-4 text-sm font-650 text-slate-950 transition-colors hover:bg-slate-50"
          >
            <Eye size={16} />
          ...
          <a
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-primary px-4 text-sm font-650 text-white transition-colors hover:bg-primary/90"
          >
            <Download size={16} />
            Descargar
          </a>
...
function LegalBasis({ result }: { result: PublicVerificationResult }) {
  return (
    <section className="rounded-lg border border-slate-200 bg-slate-50 px-5 py-5 sm:px-7">
      <h3 className="text-xs font-600 uppercase text-slate-950">Fundamento y alcance</h3>
...
function ScopeItem({
  ...
  return (
    <div className="flex min-h-[92px] items-start gap-3 border-t border-slate-200 px-5 py-4 odd:md:border-r">
      <tone.Icon size={19} className={`mt-0.5 shrink-0 ${tone.text}`} />
      <div>
        <p className="text-sm font-650 text-slate-950">{title}</p>
...
function ControlItem({
  ...
  const tone = statusTone(status);
  return (
    <div className="flex min-h-[82px] items-start gap-3 border-t border-slate-200 px-5 py-4">
      <tone.Icon size={18} className={`mt-0.5 shrink-0 ${tone.text}`} />
      <div>
        <p className="text-sm font-650 text-slate-950">{label}</p>
...
function TechnicalBlock({
  ...
  return (
    <article className="relative overflow-hidden rounded-lg border border-slate-200 bg-white px-5 py-5 sm:px-7">
      <span
        className={`absolute inset-y-0 left-0 w-1 ${accent === 'emerald' ? 'bg-emerald-600' : 'bg-primary'}`}
      />
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h4 className="text-sm font-600 uppercase text-slate-950">
            {number}. {title}
          </h4>
          <p className="mt-1 text-xs leading-5 text-slate-500">{description}</p>
        </div>
        <span
          className={`w-fit rounded-full px-2.5 py-1 text-[10px] font-600 ${valid ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}
        >
          {valid ? 'Verificado' : 'No disponible'}
        </span>
      </div>
      <pre className="mt-4 max-h-48 overflow-auto whitespace-pre-wrap break-all rounded-md border border-slate-200 bg-slate-50 p-4 font-mono text-[11px] leading-5 text-slate-700">
    ...
    <div className="flex items-start gap-3 px-5 py-4 sm:px-7">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm font-650 text-slate-950">{checkTypeLabel(check.checkType)}</p>
          <span className={`w-fit rounded-full px-2 py-1 text-[10px] font-600 ${tone.badge}`}>
...
function SectionHeader({ title, icon: Icon }: { title: string; icon: ElementType }) {
  return (
    <div className="flex items-center gap-2 border-b border-slate-200 px-5 py-4 sm:px-7">
      <Icon size={17} className="text-primary" />
      <h3 className="text-base font-650 text-slate-950">{title}</h3>
    </div>
  );
}
...
function SummaryCell({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="border-b border-slate-200 px-5 py-4 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0">
      <dt className="text-[10px] font-600 uppercase text-slate-500">{label}</dt>
      <dd
        className={`mt-1 break-words text-sm text-slate-950 ${mono ? 'font-mono text-xs' : 'font-650'}`}
      >
        {value}
      </dd>
    </div>
...
function KeyValue({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="grid gap-1 sm:grid-cols-[150px_minmax(0,1fr)]">
      <dt className="text-xs font-600 uppercase text-slate-500">{label}</dt>
      <dd className={`${mono ? 'break-all font-mono text-[11px]' : 'text-sm'} text-slate-950`}>
        {value}
      </dd>
    </div>
  );
...
function HashRow({
  ...
  return (
    <div
      className={`grid gap-2 py-4 sm:grid-cols-[130px_minmax(0,1fr)] ${bordered ? 'border-t border-slate-200' : ''}`}
    >
      <dt className="text-xs font-600 uppercase text-slate-500">{label}</dt>
      <dd className="break-all font-mono text-xs leading-5 text-slate-950">
        {value || 'No disponible'}
      </dd>
    </div>
  );
